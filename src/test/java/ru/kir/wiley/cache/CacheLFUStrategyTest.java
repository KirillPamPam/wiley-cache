...
import static ru.kir.wiley.cache.strategies.eviction.EvictionStrategy.EvictionStrategyType.LFU;

/**
 * Created by Kirill Zhitelev on 27.10.2019.
 */
public class CacheLFUStrategyTest extends CacheTest {
    private Cache<Integer, Integer> cache = new SimpleCache<>(3, LFU);
    ...
    @Test
    public void evictLeastFrequentlyUsedElementNotAsLRU() {
        cache.putObject(key0, value0);
        ...
        assertNull(cache.getObject(key2));
        assertTrue(cache.containsValue(new EvictionStrategy.Node<>(key0, value0)));
